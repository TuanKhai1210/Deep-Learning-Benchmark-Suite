...
class TestMLPClassifier(unittest.TestCase):
    ...
    def test_initialization_from_project_config(self) -> None:
        """Verify MLPClassifier builds cleanly from the project's official config module."""
        from configs.a1.models.mlp import CONFIG

        model = MLPClassifier(CONFIG['model']['parameters'])
    ...
    def test_forward_rejects_samples_with_wrong_feature_count(self) -> None:
        model = MLPClassifier(self._parameters()).eval()

        for images in (torch.randn(4, 1, 28, 27), torch.randn(4, 783)):
            with self.subTest(input_shape=tuple(images.shape)):
    ...
    def test_input_shape_polymorphism_and_output_dimensions(self) -> None:
        """Verify model handles both 4D image tensors (B, 1, 28, 28) and 2D flattened vectors (B, 784)."""
        batch_size = 8
        model = MLPClassifier(self._parameters()).eval()
        input_dim = 784
        num_classes = 10

        # 1. Test 4D image input (B, C, H, W)
        x_4d = torch.randn(batch_size, 1, 28, 28)
        out_4d = model(x_4d)
        self.assertEqual(out_4d.shape, (batch_size, num_classes))

        # 2. Test 2D flattened input (B, input_dim)
        x_2d = torch.randn(batch_size, input_dim)
        out_2d = model(x_2d)
        self.assertEqual(out_2d.shape, (batch_size, num_classes))

        # 3. Test 3D input without explicit channel (B, 28, 28)
        x_3d = torch.randn(batch_size, 28, 28)
        out_3d = model(x_3d)
        self.assertEqual(out_3d.shape, (batch_size, num_classes))
    ...
    def test_missing_required_parameters_raise(self) -> None:
        ...
        for parameter in ("input_dim", "num_classes", "hidden_dims"):
