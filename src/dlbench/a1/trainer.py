...
def generate_run_id(config: Mapping[str, Any], *, smoke: bool = False) -> str:
    ...
    return f"a1_{model_name}_{smoke_status}_seed{run_seed}_{timestamp}"
...
def fit(config: Mapping[str, Any], *, smoke: bool = False) -> FitResult:
    ...
    run_id = generate_run_id(config, smoke=smoke)
    run_dir = create_run_dir(output_root, run_id)

    # Choose device based on availability instead of the config
    device = get_device()

    # Save run metadata
    save_run_metadata(run_dir, dict(config))

    # Build data loaders
    dataloaders = build_dataloaders(config, smoke=smoke)
    ...
    epochs = int(config["budget"]["max_epochs"])
    if smoke:
    ...
    for epoch in range(epochs):
        ...
        # Save resumable state at the configured frequency and at the final epoch.
