...
class DatasetPreparationTests(unittest.TestCase):
    ...
    def test_prepare_data_initializes_missing_mean_and_std_in_config(self):
        config = {
            "data": {
                "root": "data",
                "split_file": "unused.json",
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("dlbench.a1.data.dataset.load_official_dataset", return_value=fake_dataset), \
                 patch("dlbench.a1.data.dataset.create_split", return_value=manifest), \
                 patch("dlbench.a1.data.dataset.save_split"), \
                 patch("dlbench.a1.data.dataset.compute_normalization", return_value=([0.5], [0.25])):
                prepare_data(config)

        self.assertEqual(config["preprocessing"]["mean"], [0.5])
        self.assertEqual(config["preprocessing"]["std"], [0.25])
